...
const RecentActivity = ({ recentActivities, fetchUserData, formatDate }) => {
  ...
  const toolCategories = [
    { id: 'all', name: 'All Activities', icon: FileText, color: 'bg-purple-100 text-purple-600' },
    { id: 'conversion', name: 'Conversions', icon: Zap, color: 'bg-blue-100 text-blue-600' },
    { id: 'compression', name: 'Compressions', icon: Download, color: 'bg-green-100 text-green-600' },
    { id: 'signature', name: 'Signatures', icon: FileCheck, color: 'bg-orange-100 text-orange-600' },
    { id: 'edit', name: 'Edit Tools', icon: FileText, color: 'bg-cyan-100 text-cyan-600' },
    { id: 'organize', name: 'Organize Tools', icon: FileText, color: 'bg-emerald-100 text-emerald-600' },
    { id: 'security', name: 'Security Tools', icon: FileCheck, color: 'bg-red-100 text-red-600' },
    { id: 'optimize', name: 'Optimize Tools', icon: Zap, color: 'bg-pink-100 text-pink-600' },
    { id: 'advanced', name: 'Advanced Tools', icon: FileText, color: 'bg-indigo-100 text-indigo-600' },
  ];
  ...
  const fetchRecentActivities = async () => {
    try {
      ...
      try {
        ...
        console.log('Activities endpoint not available, trying files endpoint...');
      }
        ...
        try {
          ...
          if (filesResponse.ok) {
            ...
            activitiesData = filesData.slice(0, 5).map(file => ({
              ...
              downloadUrl: file.downloadUrl || `/api/files/download/${file._id}`
            }));
          }
        } catch (error) {
  ...
  const filteredActivities = activities.filter(activity => {
    const matchesTool = selectedTool === 'all' || activity.tool === selectedTool;
    const matchesSearch = activity.fileName.toLowerCase().includes(searchTerm.toLowerCase()) ||
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {formatDate ? formatDate(activity.date) : new Date(activity.date).toLocaleDateString()}
                  </p>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="p-4 max-h-[70vh] overflow-auto">
              {previewFile.type === 'pdf' ? (
                ...
                <div className="text-center py-8">
                  ...
                  <Button 
                    onClick={() => handleDownload({ downloadUrl: previewFile.url, fileName: previewFile.name })}
                    className="mt-4"
                  >
            ...
            <div className="p-4 border-t bg-gray-50 flex justify-end">
              <Button onClick={() => handleDownload({ downloadUrl: previewFile.url, fileName: previewFile.name })}>
                <Download className="h-4 w-4 mr-2" />
                Download
              </Button>
            </div>
