...
import { Edit, Folder, Shield, Zap, Settings, RefreshCw, AlertTriangle, FileText } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useAuth } from "@/contexts/AuthContext";

const API_URL = import.meta.env.VITE_API_URL;

const ToolUsageOverview = ({ stats, planLimits, getUsagePercentage, toolUsage, user }) => {
      ...
      if (data.success) {
        ...
        if (userPlan) {
          ...
          const planWithConvertLimit = {
            ...userPlan,
            convertToolsLimit: userPlan.convertToolsLimit || getDefaultConvertLimit(userPlan.planId)
          };
          ...
          const freePlanWithConvertLimit = freePlan ? {
            ...freePlan,
            convertToolsLimit: freePlan.convertToolsLimit || getDefaultConvertLimit('free')
          } : null;
          console.log('⚠️ Using free plan as fallback:', freePlanWithConvertLimit);
  ...
  const toolCategories = [
    {
      ...
      value: `${stats.editToolsUsed} / ${
        getSafeLimit(safePlanLimits?.editToolsLimit) === 0 || getSafeLimit(safePlanLimits?.editToolsLimit) === 99999 ? "Unlimited" : getSafeLimit(safePlanLimits?.editToolsLimit)
      }`,
      usage: getUsagePercentage(stats.editToolsUsed, getSafeLimit(safePlanLimits?.editToolsLimit)),
      icon: Edit,
    ...
    },
    {
      ...
      value: `${stats.organizeToolsUsed} / ${
        getSafeLimit(safePlanLimits?.organizeToolsLimit) === 0 || getSafeLimit(safePlanLimits?.organizeToolsLimit) === 99999 ? "Unlimited" : getSafeLimit(safePlanLimits?.organizeToolsLimit)
      }`,
      usage: getUsagePercentage(stats.organizeToolsUsed, getSafeLimit(safePlanLimits?.organizeToolsLimit)),
      icon: Folder,
    ...
    },
    {
      ...
      value: `${stats.securityToolsUsed} / ${
        getSafeLimit(safePlanLimits?.securityToolsLimit) === 0 || getSafeLimit(safePlanLimits?.securityToolsLimit) === 99999 ? "Unlimited" : getSafeLimit(safePlanLimits?.securityToolsLimit)
      }`,
      usage: getUsagePercentage(stats.securityToolsUsed, getSafeLimit(safePlanLimits?.securityToolsLimit)),
      icon: Shield,
    ...
    },
    {
      ...
      value: `${stats.optimizeToolsUsed} / ${
        getSafeLimit(safePlanLimits?.optimizeToolsLimit) === 0 || getSafeLimit(safePlanLimits?.optimizeToolsLimit) === 99999 ? "Unlimited" : getSafeLimit(safePlanLimits?.optimizeToolsLimit)
      }`,
      usage: getUsagePercentage(stats.optimizeToolsUsed, getSafeLimit(safePlanLimits?.optimizeToolsLimit)),
      icon: Zap,
    ...
    },
    {
      ...
      value: `${stats.convertToolsUsed || 0} / ${
        getSafeLimit(safePlanLimits?.convertToolsLimit) === 0 || getSafeLimit(safePlanLimits?.convertToolsLimit) === 99999 ? "Unlimited" : getSafeLimit(safePlanLimits?.convertToolsLimit)
      }`,
      usage: getUsagePercentage(stats.convertToolsUsed || 0, getSafeLimit(safePlanLimits?.convertToolsLimit)),
      icon: FileText,
    ...
    },
    {
      ...
      value: `${stats.advancedToolsUsed} / ${
        getSafeLimit(safePlanLimits?.advancedToolsLimit) === 0 || getSafeLimit(safePlanLimits?.advancedToolsLimit) === 99999 ? "Unlimited" : getSafeLimit(safePlanLimits?.advancedToolsLimit)
      }`,
      usage: getUsagePercentage(stats.advancedToolsUsed, getSafeLimit(safePlanLimits?.advancedToolsLimit)),
      icon: Settings,
  ...
  const getUsageStatus = (usage, limit) => {
    if (limit === 0 || limit === 99999) return { status: 'unlimited', color: 'text-green-600' };
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Current Plan: <span className="font-semibold">{safePlanLimits?.name || user?.planName}</span>
          </p>
        </div>
        <Button 
          variant="outline" 
            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-lg font-bold text-gray-900">{tool.value}</p>
