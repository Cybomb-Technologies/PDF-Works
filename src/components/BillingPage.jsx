...
import { Check, FileText, Zap, Crown, Building2, Download, Star, Globe, X, ChevronDown, ChevronUp } from 'lucide-react';
...
const BillingPage = () => {
  ...
  const formatPeriod = (plan, isCustom = false) => {
    ...
    if (currency === 'INR') {
      ...
      const annualPrice = Math.round((plan.billingCycles?.annual || (plan.usdPrice * 12 * 0.75)) * exchangeRate);
    ...
    if (billingCycle === 'annual') {
      const annualPrice = plan.billingCycles?.annual || (plan.usdPrice * 12 * 0.75);
  ...
  const generateDetailedFeatures = (plan) => {
    ...
    features.push({
      text: `${plan.conversionLimit > 0 ? plan.conversionLimit : 'Unlimited'} PDF conversions per month`,
      available: true,
      highlight: true,
      icon: '📄'
    });
    ...
    features.push({
      text: `${plan.editToolsLimit > 0 ? plan.editToolsLimit : 'Unlimited'} Edit tools usage`,
      available: true,
      highlight: true,
      icon: '📝'
    });
    ...
    features.push({
      text: `${plan.organizeToolsLimit > 0 ? plan.organizeToolsLimit : 'Unlimited'} Organize tools usage`,
      available: true,
      highlight: true,
      icon: '📑'
    });
    ...
    features.push({
      text: `${plan.securityToolsLimit > 0 ? plan.securityToolsLimit : 'Unlimited'} Security tools usage`,
      available: true,
      highlight: true,
      icon: '🔒'
    });
    ...
    features.push({
      text: `${plan.optimizeToolsLimit > 0 ? plan.optimizeToolsLimit : 'Unlimited'} Optimize tools usage`,
      available: true,
      highlight: true,
      icon: '⚡'
    });
    ...
    features.push({
      text: `${plan.advancedToolsLimit > 0 ? plan.advancedToolsLimit : 'Unlimited'} Advanced tools usage`,
    ...
    features.push({
      text: `${plan.maxFileSize > 0 ? `${plan.maxFileSize}MB` : 'No'} file size limits`,
      available: true,
      highlight: false,
      icon: '💾'
    });
    ...
    features.push({
      text: `${plan.storage > 0 ? `${plan.storage}GB` : 'Unlimited'} cloud storage`,
      available: true,
      highlight: false,
      icon: '☁️'
    });
  ...
  const generateFeatureComparison = () => {
    ...
    const freePlan = pricingPlans.find(p => p.id === 'free') || pricingPlans[0];
    const starterPlan = pricingPlans.find(p => p.id === 'starter') || pricingPlans[1];
    const professionalPlan = pricingPlans.find(p => p.id === 'professional') || pricingPlans[2];
    const enterprisePlan = pricingPlans.find(p => p.id === 'enterprise') || pricingPlans[3];
  ...
  const faqs = [
    {
      ...
      answer: 'The free plan includes basic PDF conversions with limited features. Perfect for occasional personal use.'
    },
    {
      question: 'What file formats do you support for conversion?',
      answer: 'We support all major formats including PDF to Word, Excel, PowerPoint, Images (JPG, PNG), HTML, and vice versa. Higher plans unlock more advanced features.'
    },
    {
    ...
    },
    {
      ...
      answer: 'Yes, we offer free trials for our paid plans with full access to all features. No credit card required to start your trial.'
    },
    {
  ...
  if (pricingPlans.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <h2 className="text-2xl font-bold text-gray-600 mb-2">No Pricing Plans Available</h2>
        <p className="text-gray-500 mb-4">Please check back later or contact support.</p>
        <Button onClick={fetchPricingPlans} className="bg-blue-600 hover:bg-blue-700">
          Retry
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-12 max-w-7xl mx-auto px-4">
        ...
        {pricingPlans.map((plan, index) => {
          ...
          return (
            <motion.div
              ...
              className={`glass-effect rounded-2xl p-6 flex flex-col hover-lift relative ${plan.popular ? 'ring-2 ring-purple-500 shadow-lg scale-105' : ''} ${isFreePlan ? 'border-2 border-green-200' : ''}`}
            >
              {plan.popular && (
              ...
              <p className="text-gray-600 text-sm mb-4">{formatPeriod(plan, isEnterprise)}</p>
              <p className="text-gray-700 text-sm mb-4">{plan.description}</p>

              {/* Tool Limits Summary */}
              <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                <div className="text-sm font-semibold text-gray-700 mb-2">
                  {plan.conversionLimit > 0 ? `${plan.conversionLimit} conversions/month` : 'Unlimited conversions'}
                </div>
                <div className="grid grid-cols-2 gap-1 text-xs text-gray-600">
                  <div>📝 {plan.editToolsLimit > 0 ? `${plan.editToolsLimit} edits` : 'Unlimited edits'}</div>
                  <div>📑 {plan.organizeToolsLimit > 0 ? `${plan.organizeToolsLimit} organize` : 'Unlimited organize'}</div>
                  <div>🔒 {plan.securityToolsLimit > 0 ? `${plan.securityToolsLimit} security` : 'Unlimited security'}</div>
                  <div>⚡ {plan.optimizeToolsLimit > 0 ? `${plan.optimizeToolsLimit} optimize` : 'Unlimited optimize'}</div>
              ...
              <div className="mb-6">
                ...
                <div className={`space-y-3 transition-all duration-300 ${isExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                  {detailedFeatures.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 list-none">
                      {feature.available ? (
                        <Check className={`h-4 w-4 flex-shrink-0 mt-0.5 ${feature.highlight ? 'text-green-600' : 'text-gray-400'}`} />
                      ) : (
                        <X className="h-4 w-4 flex-shrink-0 mt-0.5 text-red-400" />
                      )}
                      <span className={`text-xs flex items-center gap-2 ${feature.available ? (feature.highlight ? 'text-gray-800 font-medium' : 'text-gray-600') : 'text-gray-400 line-through'}`}>
              ...
              <Button onClick={() => handleUpgrade(plan.id)} disabled={isCurrentPlan} className={`w-full ${isCurrentPlan ? 'bg-gray-300 cursor-not-allowed' : isFreePlan ? 'bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700' : `bg-gradient-to-r ${plan.color} hover:opacity-90`}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-effect rounded-2xl p-6">
        <h2 className="text-2xl font-bold text-center mb-6 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Compare All Features</h2>
            ...
            {featureComparison.map((item, index) => (
              <div key={index} className={`grid grid-cols-5 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                ...
                <div className="p-3 text-center border-r text-sm">
                  {typeof item.free === 'boolean' ? (item.free ? <Check className="h-4 w-4 text-green-600 mx-auto" /> : <X className="h-4 w-4 text-red-400 mx-auto" />) : <span className="text-gray-700">{item.free}</span>}
                </div>
                <div className="p-3 text-center border-r text-sm">
                  {typeof item.starter === 'boolean' ? (item.starter ? <Check className="h-4 w-4 text-green-600 mx-auto" /> : <X className="h-4 w-4 text-red-400 mx-auto" />) : <span className="text-gray-700">{item.starter}</span>}
                </div>
                <div className="p-3 text-center border-r text-sm">
                  {typeof item.professional === 'boolean' ? (item.professional ? <Check className="h-4 w-4 text-green-600 mx-auto" /> : <X className="h-4 w-4 text-red-400 mx-auto" />) : <span className="text-gray-700">{item.professional}</span>}
                </div>
                <div className="p-3 text-center text-sm">
                  {typeof item.enterprise === 'boolean' ? (item.enterprise ? <Check className="h-4 w-4 text-green-600 mx-auto" /> : <X className="h-4 w-4 text-red-400 mx-auto" />) : <span className="text-gray-700">{item.enterprise}</span>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div className={`h-3 rounded-full ${percentage >= 90 ? 'bg-red-500' : percentage >= 75 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${Math.min(percentage, 100)}%` }}></div>
              </div>
              <p className="text-xs text-gray-500 mt-2">{percentage >= 90 ? 'You ve reached your limit. Upgrade to continue converting.' : percentage >= 75 ? 'Youre approaching your limit. Consider upgrading for more conversions.' : `You're on the ${planName} plan.`}</p>
            </div>
          </div>
        </motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass-effect rounded-2xl p-6">
        <h2 className="text-2xl font-bold text-center mb-6 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Frequently Asked Questions</h2>
        <div className="space-y-3 max-w-4xl mx-auto">
          {faqs.map((faq, index) => (
            <div key={index} className="border border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors bg-white">
              <div className="p-4 flex justify-between items-center hover:bg-blue-50 rounded-lg transition-colors" onClick={() => toggleFaq(index)}>
              ...
              {expandedFaq === index && <div className="p-4 pt-0 text-gray-700 text-sm border-t">{faq.answer}</div>}
            </div>
          ))}
        </div>
      </motion.div>
      ...
      {/* Final CTA Section */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="text-center py-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-3xl text-white">
        <div className="flex justify-center mb-4"><Download className="h-10 w-10 text-white opacity-90" /></div>
        <h2 className="text-2xl font-bold mb-3">Ready to Transform Your PDF Workflow?</h2>
        <p className="text-blue-100 text-sm mb-4 max-w-2xl mx-auto">Start with our free plan today. No credit card required. Upgrade anytime.</p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Button onClick={() => handleUpgrade('free')} className="bg-white text-blue-600 hover:bg-blue-50 px-6 py-2 font-semibold rounded-full">Start Free Plan</Button>
          <Button onClick={() => handleUpgrade('professional')} className="bg-transparent border-2 border-white text-white hover:bg-white hover:text-blue-600 px-6 py-2 font-semibold rounded-full">Try Professional Free</Button>
        </div>
        <p className="text-blue-200 text-xs mt-3">{currency === 'INR' ? 'Prices in INR include taxes • ' : ''}14-day free trial on paid plans • No commitment</p>
      </motion.div>
    </div>
  );
};
