...
router.post("/upload", verifyToken, upload.single("pdfFile"), EditController.uploadPDF);
...
router.post("/save-pdf-edit", verifyToken, upload.single("file"), EditController.savePdfEdit);
router.post("/save-image-crop", verifyToken, upload.single("file"), EditController.saveImageCrop);

// File rename with batch support - PROTECTED WITH AUTH
router.post("/save-file-rename", verifyToken, upload.array("files", 10), EditController.saveFileRename);

// Batch download route - PROTECTED WITH AUTH
router.get("/download-batch/:batchId", verifyToken, EditController.downloadBatch);
