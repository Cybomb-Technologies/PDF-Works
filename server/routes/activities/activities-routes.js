...
router.get("/", verifyToken, async (req, res) => {
  try {
    ...
    // Define all model queries with proper field selection
    const queries = [
      Edit.find({ userId })
...
function normalizeActivityData(activity, toolName) {
  try {
    ...
    return {
      ...
      fileSize: Number(activity.fileSize) || 0,
      metadata: activity
    };
  } catch (error) {
...
// Helper function to get tool name by index
function getToolNameByIndex(index) {
  const tools = ['edit', 'organize', 'security', 'optimize', 'advanced', 'convert'];
...
router.get("/download/:tool/:filename", verifyToken, async (req, res) => {
  try {
    ...
    // Define upload directories for each tool
    const uploadDirs = {
...
router.get("/stats", verifyToken, async (req, res) => {
  try {
    ...
    const toolNames = ['edit', 'organize', 'security', 'optimize', 'advanced', 'convert'];
    const result = {};
