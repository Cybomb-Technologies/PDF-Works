...
userSchema.methods.shouldResetUsage = function() {
  const now = new Date();
  const resetDate = this.usage?.resetDate ? new Date(this.usage.resetDate) : null;
  ...
  try {
    ...
    if (!plan && this.planName) {
      plan = await PricingPlan.findOne({
        $or: [
          { planId: this.planName.toLowerCase() },
          { name: this.planName }
        ]
      });
    }
...
userSchema.methods.getDaysRemainingInCycle = function() {
  const now = new Date();
  const resetDate = this.usage?.resetDate ? new Date(this.usage.resetDate) : new Date(this.createdAt);
