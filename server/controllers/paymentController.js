...
const createOrder = async (req, res) => {
  try {
    ...
    const { amount, currency: orderCurrency, plan } = await calculateAmountFromPlan(
      planId,
      billingCycle,
      currency
    );
...
const getBillingHistory = async (req, res) => {
  try {
    ...
    .sort({ createdAt: -1 })
    .limit(20)
    .select('transactionId amount currency status planName billingCycle createdAt expiryDate paymentMethod paidAt');
